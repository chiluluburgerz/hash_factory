...
// Security:
//   - Auth required
//   - tenant_admin OR system admin required for all write routes
...
//     publication semantics, and topic routing.
// ============================================================================
...
export type MerkleWriteRoutesOpts = Readonly<{
  merkle: MerkleClient;
}>;

const merkleWriteRoutes: FastifyPluginAsync<MerkleWriteRoutesOpts> = async (app, opts) => {
  if (!opts?.merkle) throw new Error("merkleWriteRoutes requires merkle client");
  ...
  app.post("/v1/merkle/root/public/publish", { preHandler: requireAuth }, async (req, reply) => {
    const actor = requireActor(req);
    requireTenantAdminOrSystem(actor);
    const body = requireBodyObject(req);
    try {
  ...
  app.post("/v1/merkle/root/public/unpublish", { preHandler: requireAuth }, async (req, reply) => {
    const actor = requireActor(req);
    requireTenantAdminOrSystem(actor);
    const body = requireBodyObject(req);
    try {
  ...
  app.post("/v1/merkle/proof/build-store", { preHandler: requireAuth }, async (req, reply) => {
    const actor = requireActor(req);
    requireTenantAdminOrSystem(actor);
    const body = requireBodyObject(req);
    try {
...
export { merkleWriteRoutes };
export default merkleWriteRoutes;