// ============================================================================
// File: tests/units/datasets/scan.test.ts
// Version: 1.0.0-hf-datasets-scan-unit | 2026-03-07
// Purpose:
//   Unit tests for src/datasets/scan.ts
// Notes:
...
vi.mock("../../../../src/datasets/pathNorm.js", () => ({
...
describe("datasets/scan (unit)", () => {
  ...
  it("rejects missing root directory", async () => {
    const { scanDataset } = await import("../../../../src/datasets/scan.js");

    lstatMock.mockRejectedValue(new Error("ENOENT"));

    await expect(scanDataset("", {} as any)).rejects.toMatchObject({
      name: "DatasetError",
  ...
  it("rejects root when it is not a directory", async () => {
    ...
    await expect(
      scanDataset(path.resolve("/tmp/file.txt"), {} as any),
  ...
  it("scans files deterministically and sorts final output by normalized relative path", async () => {
    ...
    const out = await scanDataset(ROOT, undefined, progress);
  ...
  it("rejects invalid allowed_suffixes shape", async () => {
    ...
    await expect(
      scanDataset(ROOT, { allowed_suffixes: ".tsv" as any }),
    ).rejects.toMatchObject({
  ...
  it("filters files by allowed suffixes", async () => {
    ...
    const progress = vi.fn();
    const out = await scanDataset(ROOT, { allowed_suffixes: [".tsv"] } as any, progress);
  ...
  it("follows in-root symlinked files when enabled", async () => {
    ...
    const out = await scanDataset(ROOT, { follow_symlinks: true } as any);
  ...
  it("rejects symlinks that escape the root", async () => {
    ...
    await expect(
      scanDataset(ROOT, { follow_symlinks: true } as any),
    ).rejects.toMatchObject({
  ...
  it("enforces max_single_file_bytes", async () => {
    ...
    await expect(
      scanDataset(ROOT, { max_single_file_bytes: 100 } as any),
    ).rejects.toMatchObject({
  ...
  it("enforces max_total_bytes", async () => {
    ...
    await expect(
      scanDataset(ROOT, { max_total_bytes: 100 } as any),
    ).rejects.toMatchObject({
  ...
  it("enforces max_files", async () => {
    ...
    await expect(
      scanDataset(ROOT, { max_files: 1 } as any),
    ).rejects.toMatchObject({
  ...
  it("throws SCAN_EMPTY when no files survive filtering", async () => {
    ...
    await expect(scanDataset(ROOT, undefined)).rejects.toMatchObject({
